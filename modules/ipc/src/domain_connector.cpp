...
const connector::argument connector::default_arguments_ = {};
...
void connector::initialize(const std::string& domain_path,
                           const argument&    args)
{
  last_arguments_ = args;
  last_domain_    = domain_path;
}

bool connector::connect(const std::string& domain_path,
                        const argument&    args)
{
  last_arguments_ = args;
  last_domain_    = domain_path;
  ...
  try
  {
    ...
    // connection
    {
      ...
      if (socket_.connect(domain_path) == false)
      {
...
ssize_t connector::send(const uint8_t* data, size_t size, int send_flags, int retry_flag)
{
  // check connection
  if (connected_ == false && connect(last_domain_, last_arguments_) == false)
    return -1;

  // send data
  ssize_t res = try_send(data, size, send_flags);
  ...
  if (res == 0 && retry)  // 전송 실패 원인이 Connection 관련이라면 접속 후 재전송
  {
    ...
    if (connect(last_domain_, last_arguments_))
...
ssize_t connector::send(const rs::Packet& packet, int send_flags, int retry_flag)
{
...
ssize_t connector::recv(uint8_t* data, size_t size, int recv_flags)
{
  if (connected_ == false)
  {
...
ssize_t connector::sendToRecv(const uint8_t* send_data, const size_t send_size,
...
ssize_t connector::try_send(const uint8_t* data, size_t size, int send_flags)
{
